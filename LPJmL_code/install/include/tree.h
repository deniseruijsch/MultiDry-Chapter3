...
typedef struct
{
  ...
  Real gddtw;
  Real drought_cum;   // cumulative daily drought stress
  Real drought_memory;       // new: drought memory for mortality
  Real mort_background_annual;
  Real mort_heat_annual;
  Real nind_kill_cum;
  Real nind_start_of_year;
  ...
  int boll_age;
  Stocks bm_inc_prev;   /**< previous month's biomass increment for monthly mortality calculation */
} Pfttree;
...
extern void mortality_monthly_tree(Litter *, Pft *, Real,Bool,const Config *);
...
extern void litter_update_mortality_tree(Litter *, Pft *, Real, const Config *);
